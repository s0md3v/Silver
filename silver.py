...
parser.add_argument(
    "--resolve",
    help="txt file contains hostnames too",
    dest="resolve",
    action="store_true",
)
...
if exclude:
    ...
    exclude = ""
os.system(
    "masscan%s-p%s --rate %i -oG %s %s %s >/dev/null 2>&1"
    % (host, ports_to_scan, args.rate, savefile, hostfile, exclude)
)
...
for host in master_db:
    if host not in exclude:
        if (
            len(master_db[host]) >= core.memory.config["shodan_call_threshold"]
            and core.memory.config["shodan_api_key"]
        ):
